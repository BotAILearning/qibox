...
test('provider page renders legacy single model as 默认模型 with all features assigned', () => {
  const state = { provider: { baseUrl: 'https://x.test/v1', model: 'm1', protocol: 'openai', timeout: 30, consent: true, hasKey: true, tested: true }, models: [], assignments: { chat: 'legacy', proactive: 'legacy', learning: 'legacy', analysis: 'legacy' }, schema: { providerPresets: presets } };
  const html = providerPage(state, null);
  assert.match(html, /默认模型/);
  assert.match(html, /聊天回复/); assert.match(html, /主动聊天/); assert.match(html, /风格学习/); assert.match(html, /聊天分析/);
  ...
  assert.ok((html.match(/value="legacy"/g) || []).length >= 4); // 4 feature dropdowns
});
...
test('provider page renders multi-model list and editor', () => {
  ...
  const html = providerPage(state, null);
  assert.match(html, /模型A/); assert.match(html, /模型B/);
  assert.match(html, /用于：聊天回复/); assert.match(html, /未分配功能/);
  ...
  assert.match(edit, /id="ai-model-form"/); assert.match(edit, /name="label"/); assert.match(edit, /id="ai-model-preset"/); assert.match(edit, /data-ai-action="test"/); assert.match(edit, /保存模型/);
});
