...
test('第一次添加的模型默认应用于全部功能，且密钥加密保存', async t => {
  ...
  assert.deepEqual(state.assignments, { chat: id, proactive: id, learning: id, analysis: id });
...
test('功能可按需分配不同模型，运行时按功能取模型', async t => {
  ...
  const saved = await a.saveModels({ models: [{ id: m1, label: '模型A', ...modelA }, { id: 'draft-b', label: '模型B', ...modelB }], assignments: { chat: m1, proactive: 'draft-b', learning: m1, analysis: 'draft-b' } });
  const m2 = saved.models.find(m => m.id !== m1).id;
  assert.equal(m2, 'draft-b');                      // 客户端草稿 id 被保留
  assert.equal(saved.assignments.chat, m1);
  assert.equal(saved.assignments.proactive, 'draft-b'); // 功能可分配到具体模型
  assert.equal(saved.assignments.learning, m1);
  assert.equal(saved.assignments.analysis, 'draft-b');
  assert.equal(a.modelFor('chat').apiKey, K1);
  assert.equal(a.modelFor('proactive').apiKey, K2);
  assert.equal(a.modelFor('learning').apiKey, K1);
...
test('修改模型配置后清除已验证状态，未修改则保留', async t => {
  ...
  await a.saveModels({ models: [{ id, label: '模型A', ...modelA }], assignments: { chat: id, proactive: id, learning: id, analysis: id } });
  assert.equal(a.publicState().models[0].tested, true);
  await a.saveModels({ models: [{ id, label: '模型A', ...modelA, model: 'changed-model' }], assignments: { chat: id, proactive: id, learning: id, analysis: id } });
...
test('删除模型后，引用它的功能回退到剩余第一个模型', async t => {
  ...
  const a = new AIAssistant({ dataRoot: root, bridge, provider }); await a.init();
  t.after(async () => { await a.close(); await cleanup(root); });
  const state = await a.saveModels({ models: [{ label: '模型A', ...modelA }, { label: '模型B', ...modelB }], assignments: { chat: null, proactive: null, learning: null, analysis: null } });
  const m1 = state.models[0].id, m2 = state.models[1].id;
  const state2 = await a.saveModels({ models: [{ id: m2, label: '模型B', ...modelB }], assignments: { chat: m1, proactive: m1, learning: m1, analysis: m2 } });
  assert.equal(state2.models.length, 1);
  assert.equal(state2.assignments.chat, m2);
  assert.equal(state2.assignments.analysis, m2);
...
test('测试连接成功后，编辑同名模型可保留已存密钥再测', async t => {
  ...
  const state2 = await a.saveModels({ models: [{ id, label: '模型A', ...stored }], assignments: { chat: id, proactive: id, learning: id, analysis: id } });
...
test('草稿模型测试通过后保存保留已验证状态；改配置未重测则清除', async t => {
  ...
  assert.equal(state.models[0].tested, true);
  assert.equal(a.modelFor('chat').apiKey, K1);
  const state2 = await a.saveModels({ models: [{ id: draftId, label: '草稿A', ...modelA, model: 'changed-model' }], assignments: { chat: draftId, proactive: draftId, learning: draftId, analysis: draftId } });
  assert.equal(state2.models[0].tested, false);
});
