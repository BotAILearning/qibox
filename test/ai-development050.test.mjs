...
test('nonempty learned memory is preserved without a lock; cleared memory can be filled by a new learning', async t => {
  ...
  provider.next = async () => ({ style: learningStyle, memory: { summary: '新候选' } });
  ...
  provider.next = async () => ({ style: learningStyle, memory: { summary: '空内容重新填入' } });
...
test('逐人分析读取精确日期、独立保存报告且不发送聊天消息', async t => {
  ...
  assert.equal(modelCalls, 2, 'one provider call per contact');
