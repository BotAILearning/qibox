...
export function replyRecordCards(state, filters, records, loading, activityEntries) {
  ...
  const cards = entries.slice(page * 25, page * 25 + 25).map((profile, index) => {
    ...
    return `<article class="ai-reply-record-card" data-ai-reply-card="${esc(profile.id)}">
      ...
      <details data-ai-record-expand="${esc(profile.id)}" ${filters.expanded?.includes(profile.id) ? 'open' : ''}><summary>执行详情 <span>${messages.length} 条记录</span></summary><div class="ai-reply-record-detail">${profile.needsHelp ? `<div class="ai-reply-record-help"><span>${esc(profile.reason || '需要本人处理')}</span>${profile.queueFailed ? '<button class="quiet" type="button" data-ai-nav="proactive">查看主动聊天任务</button>' : `<button class="quiet" type="button" data-ai-resume-profile="${esc(profile.id)}">开启自动回复</button>`}</div>` : ''}<div class="ai-reply-summary-toolbar"><span>按时间范围生成聊天总结</span><div class="ai-summary-controls"><select data-ai-summary-range="${esc(profile.id)}" aria-label="${esc(name)}的总结时间范围"><option value="takeover">本次接管</option><option value="all">全部</option><option value="day">近一天</option><option value="week">近一周</option><option value="month">近一月</option></select><button type="button" class="primary" data-ai-summary-profile="${esc(profile.id)}">总结</button></div></div><p class="ai-summary-result" data-ai-summary-result="${esc(profile.id)}" role="status" hidden></p><section class="ai-reply-history"><header><h4>执行记录</h4><span>${messages.length} 条</span></header>${history}</section></div></details>
    </article>`;
  }).join('');
