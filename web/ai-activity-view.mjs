...
export function activityRows(state, filters, records, loading) {
  ...
  return `${failures.length ? `<div class="ai-record-error" role="alert">${failures.map(r => `<p>${esc(r.label)}：${esc(r.error || '记录读取失败，请重试')}</p>`).join('')}<button type="button" class="secondary" data-ai-retry-records>重新读取</button></div>` : ''}<div class="ap-table-scroll"><table class="ai-record-table ap-record-table ap-reply-record-table"><thead><tr><th>联系人</th><th>最近执行时间</th><th>执行记录</th><th>操作</th></tr></thead><tbody>${entries.slice(page * 25, page * 25 + 25).map(p => {
    ...
    const open = `<button class="secondary ap-record-open" type="button" data-ai-open-conversation="${esc(p.id)}">打开聊天</button>`;
    return `<tr class="ai-contact-record"><td data-label="联系人"><strong>${contactName(p)}</strong><small>${p.kind === 'group' ? '群聊' : '联系人'}</small></td><td data-label="最近执行时间">${esc(beijingTime(messages[0]?.at || p.at))}</td><td data-label="执行记录">${p.needsHelp ? `<p class="ai-help">${esc(p.reason)}</p>${p.queueFailed ? '<button class="quiet" type="button" data-ai-nav="proactive">查看主动聊天任务</button>' : `<button class="quiet" type="button" ${p.needsReview ? 'data-ai-review' : 'data-ai-resume-profile'}="${esc(p.id)}">${p.needsReview ? '核验发送结果' : '开启自动回复'}</button>`}` : ''}<details data-ai-record-expand="${esc(p.id)}" ${filters.expanded?.includes(p.id) ? 'open' : ''}><summary>${source?.pending ? '正在读取历史正文…' : source?.unavailable ? '正文暂不可读取' : `展开近期执行记录（${messages.length} 条）`}</summary>${messages.length ? `<ol>${messages.map(m => m.id && m.confirmed !== false ? `<li data-ai-record-menu="${esc(m.id)}" data-ai-record-menu-source="${esc(filters.source === 'unknown' ? 'unknown' : 'reply')}"><time>${esc(beijingTime(m.at))}</time><button type="button" class="ai-record-message" data-ai-locate-message="${esc(m.id)}" data-profile-id="${esc(p.id)}" title="定位到聊天位置">${esc(m.text)}</button><button type="button" class="quiet danger-link" data-ai-delete-record="${esc(m.id)}" data-ai-delete-source="${esc(filters.source === 'unknown' ? 'unknown' : 'reply')}">删除记录</button></li>` : `<li><time>${esc(beijingTime(m.at))}</time><p>${esc(m.text)}</p><span>${filters.source === 'unknown' ? '来源未分类' : m.confirmed === false ? '待核对' : '已回复'}</span>${open}<button type="button" class="quiet danger-link" data-ai-delete-record="${esc(m.id)}" data-ai-delete-source="${esc(filters.source === 'unknown' ? 'unknown' : 'reply')}">删除记录</button></li>`).join('')}</ol>` : `<p class="ai-help">${loading ? '正在读取代发内容…' : source?.unavailable ? '暂时无法读取正文，可以直接打开聊天。' : '当前可读取范围内暂无正文，可以打开聊天查看。'}</p>`}</details><div class="ai-summary-controls"><select data-ai-summary-range="${esc(p.id)}" aria-label="总结时间范围"><option value="takeover">本次接管</option><option value="all">全部</option><option value="day">近一天</option><option value="week">近一周</option><option value="month">近一月</option></select><button type="button" class="quiet" data-ai-summary-profile="${esc(p.id)}">总结</button><p data-ai-summary-result="${esc(p.id)}" hidden></p></div></td><td data-label="操作">${open}</td></tr>`;
...
export function proactiveRecordRows(state, filters = {}, loading = false) {
  ...
  return `<div class="ap-table-scroll"><table class="ai-record-table ap-record-table ap-proactive-record-table"><thead><tr><th>任务</th><th>联系人</th><th>最近执行时间</th><th>本次执行内容</th><th>操作</th></tr></thead><tbody>${records.map(r => {
    ...
    return `<tr data-proactive-record="${esc(r.id)}"><td data-label="任务"><strong>${esc(r.taskName || '历史任务')}</strong></td><td data-label="联系人">${contactName(r) || esc(r.contact || '联系人')}</td><td data-label="最近执行时间">${esc(beijingTime(r.at))}</td><td data-label="本次执行内容"><span class="ap-record-status ${esc(r.status)}">${esc(labels[r.status] || '状态待更新')}</span>${r.segmentsTotal ? `<small>已确认 ${r.segmentsSent || 0}/${r.segmentsTotal} 段</small>` : ''}${r.text ? `${r.messageId && profileId ? `<button type="button" class="ai-record-message" data-ai-locate-message="${esc(r.messageId)}" data-profile-id="${esc(profileId)}" title="定位到聊天位置">${esc(r.text)}</button><button type="button" class="quiet" data-ai-locate-message="${esc(r.messageId)}" data-profile-id="${esc(profileId)}">定位到聊天位置</button>` : `<p class="ap-record-text">${esc(r.text)}</p>`}` : `<p class="ai-help">${r.bodyUnavailable ? '正文暂时无法读取，请打开聊天核对。' : '无可展示正文'}</p>`}${r.reason ? `<p class="ai-help">${esc(r.reason)}</p>` : ''}${r.status === 'uncertain' ? '<p class="ai-help">发送结果待核对，不会自动重发本条。</p>' : ''}</td><td data-label="操作">${profile?.delivery?.status === 'uncertain' ? `<button type="button" class="quiet" data-ai-review="${esc(profileId)}">核验发送结果</button>` : ''}${profileId ? `${r.status === 'uncertain' ? `<button type="button" class="quiet" data-ai-review="${esc(profileId)}">核对结果</button>` : ''}<button type="button" class="quiet" data-ai-open-conversation="${esc(profileId)}">打开聊天</button>` : '<span class="ai-help">联系人暂不可定位</span>'}<button type="button" class="quiet danger-link" data-ai-delete-record="${esc(r.id)}" data-ai-delete-source="proactive">删除记录</button></td></tr>`;
...
export function skipRecordsView(state) {
  ...
  const rows = [...merged.values()].sort((a, b) => b.at - a.at).slice(0, 50).map(event => {
    ...
    return `<tr><td data-label="联系人">${esc(name)}</td><td data-label="时间">${esc(beijingTime(event.at))}</td><td data-label="原因与来源">${esc(source)} · ${esc(reason)}</td><td data-label="触发消息">${event.messageId && profile ? `<button type="button" class="quiet" data-ai-locate-message="${esc(event.messageId)}" data-profile-id="${esc(profile.id)}">定位触发消息</button>` : '<span class="ai-help">无可定位的历史消息</span>'}<button type="button" class="secondary ap-record-mark" data-ai-mark-reply="${esc(profile?.id || '')}" data-message-id="${esc(event.messageId || '')}" data-event-id="${esc(event.id || '')}" ${event.messageId ? '' : 'disabled'}>标记为需回复</button><button type="button" class="quiet danger-link" data-ai-delete-record="${esc(event.id || '')}" data-ai-delete-source="skip">删除记录</button></td></tr>`;
  }).join('');
  return `<div id="ai-skip-records"><div class="ap-table-scroll"><table class="ai-record-table ap-record-table ap-skip-record-table"><thead><tr><th>联系人</th><th>时间</th><th>原因与来源</th><th>触发消息</th></tr></thead><tbody>${rows || '<tr><td colspan="4"><div class="ap-empty">暂无未回复记录</div></td></tr>'}</tbody></table></div></div>`;
}
