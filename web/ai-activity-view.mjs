...
export function activityEntries(state, filters = {}) {
  return ((filters.source === 'unknown' ? state.activityHistory : state.activity) || []).map(p => {
    ...
    return (p.hasSent || p.needsHelp) &&
      (!filters.query || contactSearch(p).includes(filters.query.toLocaleLowerCase())) &&
...
export function activityRows(state, filters, records, loading) {
  const entries = activityEntries(state, filters), page = Math.min(Number(filters.page) || 0, Math.max(0, Math.ceil(entries.length / 25) - 1));
...
export function proactiveRecordEntries(state, filters = {}) {
  return (state.proactiveRecords || []).filter(r => (!filters.taskId || r.taskId === filters.taskId) && within(r.at, filters) &&
    (!filters.query || contactSearch(r).includes(filters.query.toLocaleLowerCase())) &&
...
export function recentErrorsBox(state, open = false, loading = false) {
  ...
  if (!total) return '';
  return `<details class="ap-record-errors"${open ? ' open' : ''}><summary><h4>最近异常（${total}）</h4><button type="button" class="quiet danger-link" data-ai-clear-errors>清空</button></summary><ul>${errors.map(e => `<li><time>${esc(beijingTime(e.at))}</time><span>${esc(e.message || 'AI 操作未完成')}</span></li>`).join('')}</ul><footer class="ap-record-footer"><span>已加载 ${errors.length} / ${total} 条 · 北京时间</span>${hasMore ? `<button class="secondary" type="button" data-ai-error-more ${loading ? 'disabled' : ''}>${loading ? '正在读取…' : '加载更早异常'}</button>` : '<span>已全部加载</span>'}</footer></details>`;
}
export function activityPage(state, filters = {}, records = [], loading = false, proactiveLoading = false, errorLoading = false) {
  ...
  const entries = activityEntries(state, filters), pages = Math.max(1, Math.ceil(entries.length / 25)), page = Math.min(Number(filters.page) || 0, pages - 1);
  return `<section class="ai-activity-page"><div class="ai-page-heading"><div><h3>运行记录</h3></div></div><div class="ap-record-tabs" role="group" aria-label="记录来源">${[['reply', '自动回复'], ['proactive', '主动聊天']].map(([key, label]) => `<button type="button" data-ai-record-source="${key}" aria-pressed="${(filters.source || 'reply') === key}">${label}</button>`).join('')}</div><form id="ai-log-filter" ${filters.open ? '' : 'hidden'}><label class="ai-field">开始日期（北京时间）<input name="from" type="date" value="${esc(filters.from)}"></label><label class="ai-field">结束日期（北京时间）<input name="to" type="date" value="${esc(filters.to)}"></label><label class="ai-field">对象<input name="query" type="search" placeholder="搜索对象" value="${esc(filters.query)}"></label><label class="ai-field">类型<select name="kind">${option('', '全部', !filters.kind)}${option('person', '联系人', filters.kind === 'person')}${option('group', '群聊', filters.kind === 'group')}</select></label><label class="ai-field">内容<select name="code">${option('', '全部', !filters.code)}${option('sent', '已代发', filters.code === 'sent')}${option('help', '需要本人处理', filters.code === 'help')}</select></label><div class="ai-filter-actions"><button type="submit" class="primary">筛选并刷新</button></div></form>
  ...
  ${filters.source === 'reply' ? `<section class="ap-record-block"><header><h4>${filters.source === 'unknown' ? '历史代发记录（来源未分类）' : '自动回复'}</h4><span>按联系人查看近期可读取内容，每页最多 25 位${filters.source === 'unknown' ? ' · 旧记录无法确认由哪种功能发出' : ''}</span><button type="button" class="icon-button" data-ai-toggle-filters aria-expanded="${!!filters.open}" aria-controls="ai-log-filter" title="筛选" aria-label="筛选">${icon('filter')}</button></header><div id="ai-activity-entries">${activityRows(state, filters, records, loading)}</div><div class="ai-actions"><button class="quiet" type="button" data-ai-log-page="${page - 1}" ${page === 0 ? 'disabled' : ''}>上一页</button><span>${page + 1} / ${pages}</span><button class="quiet" type="button" data-ai-log-page="${page + 1}" ${page + 1 >= pages ? 'disabled' : ''}>下一页</button></div></section>` : ''}<div id="ai-recent-errors">${recentErrorsBox(state, filters.errorsOpen, errorLoading)}</div></section>`;
}
