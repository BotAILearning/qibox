...
const features = [
  { key: 'chat', label: '聊天回复', hint: '自动回复收到的消息' },
  { key: 'proactive', label: '主动聊天', hint: '按目标主动发起聊天' },
  { key: 'learning', label: '风格学习', hint: '分析聊天并学习表达风格' },
  { key: 'analysis', label: '聊天分析', hint: '生成聊天分析报告' },
];
...
function assignmentsFor(state, draft) {
  ...
  return { chat: first, proactive: first, learning: first, analysis: first };
}
...
function emptyCard(state, draft) {
  return `<section class="ai-card ai-model-editor">
  <div class="ai-card-heading"><h4>模型设置</h4></div>
  <p class="ai-help">为聊天回复、主动聊天、风格学习、聊天分析分别选择模型。添加模型后，可在右侧为各功能分配模型并点击“保存”生效。</p>
  <div class="ai-model-save"><p class="ai-help">API Key 加密保存；删除模型不影响其他已保存的模型。</p><button type="button" class="primary" data-ai-action="model-add">${icon('plus')}添加模型</button></div>
  </section>`;
}
...
export function providerPage(state, draft) {
  ...
  return `<div class="ai-page-heading"><div><h3>模型设置</h3><p>为聊天回复、主动聊天、风格学习、聊天分析分别选择模型；第一个添加的模型默认应用于全部功能。</p></div>${badge}</div>
  <div class="ai-model-workspace">
    <aside class="ai-model-sidebar">
      <button type="button" class="secondary ai-model-add" data-ai-action="model-add">${icon('plus')}添加模型</button>
      ...
      <p class="ai-help">API Key 加密保存；删除模型不影响其他已保存的模型。</p>
    </aside>
    <div class="ai-model-main">${draft?.editing ? editorCard(state, draft, presets) : emptyCard(state, draft)}${assignmentCard(state, draft, models, assignments)}</div>
  </div>`;
}
