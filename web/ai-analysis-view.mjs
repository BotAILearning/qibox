...
function countLabel(report) {
  const range = report.actualRange ? ` · ${reportDate(report.actualRange)}` : '';
  return report.rangeCount > report.count ? `本次分析最近 ${report.count} 条（范围内共 ${report.rangeCount} 条）${range}` : `${report.count} 条范围内聊天记录${range}`;
}
...
export function analysisPage(state, draft, result, search = '', report = null) {
  ...
  return `<div class="ai-page-heading"><div><h3>聊天数据分析</h3><p>选择联系人和时间范围，按你的要求分别生成报告。</p></div></div>
  <form id="ai-analysis-form"><section class="ai-analysis-selection"><div class="ai-card-heading"><h4>选择联系人</h4><small id="ai-analysis-count">${selected.size} / 10</small></div>${state.contacts.some(c => c.kind === 'person') ? `<label class="ai-analysis-search">${icon('search')}<span class="sr-only">搜索联系人</span><input id="ai-analysis-search" type="search" placeholder="搜索联系人…" value="${esc(search)}"></label>` : ''}<fieldset id="ai-analysis-contacts" class="ai-analysis-contacts"><legend class="sr-only">选择联系人</legend>${analysisContactList(state, selected, search)}</fieldset><button type="button" class="secondary ai-wide" data-ai-action="scan">刷新联系人</button></section>
  <div class="ai-analysis-main"><section class="ai-analysis-request"><div class="ai-analysis-request-heading"><h4>分析要求</h4>${dateRangeField('analysis', draft, { compact: true })}</div><div class="ai-style-pills ai-analysis-presets" role="group" aria-label="分析方向快捷输入">${presetChips(draft.request)}</div><label class="ai-field"><span class="sr-only">分析要求</span><textarea name="request" maxlength="4000" rows="4" placeholder="可留空；例如：总结近期讨论的事项、已达成的约定，以及需要跟进的问题">${esc(draft.request)}</textarea></label><div class="ai-provider-footer"><p class="ai-help">报告可分别复制全文</p><button type="submit" class="primary">开始分析</button></div></section>
  ${result ? `<div class="ai-analysis-reports"><div class="ai-page-heading"><div><h3>分析报告</h3><p>${analysisRangeLabel(result)} · ${result.reports.length} 位联系人</p></div></div>${result.reports.map((r, index) => `<article class="ai-card" data-analysis-report="${index}"><div class="ai-card-heading"><div><h3>${contactName(r)}</h3>${r.status !== 'error' ? `<p class="ai-help">${countLabel(r)}</p>` : ''}</div>${r.status === 'complete' || r.status === 'empty' ? `<button type="button" class="secondary" data-ai-copy-report="${index}">复制全文</button>` : ''}</div>${r.status === 'error' ? `<p role="alert">${esc(r.error)}</p>` : `${r.truncated ? `<p role="alert" class="ai-help">${r.truncatedReasons?.includes('message_length') ? '部分超长消息只保留了可安全读取的文字片段。' : r.truncatedReasons?.includes('message_limit') ? '消息数量达到读取上限，仅覆盖读取到的部分。' : '读取输出达到安全容量，仅覆盖读取到的部分。'}</p>` : ''}${metricsView(r.metrics)}${excerptsView(r.excerpts)}<div class="ai-report-text">${reportBody(r.report)}</div>`}</article>`).join('')}</div>` : '<div class="ai-analysis-ready"><h4>准备好后，开始分析</h4><p>每位联系人的数据独立分析，结果会显示在这里。</p></div>'}</div></form>${historyList(state)}`;
}
