...
export function analysisPage(state, draft, result, search = '', report = null, rangeMode = 'all') {
  ...
  <section class="ai-analysis-request"><div class="ai-analysis-section-title"><div><h4>分析要求</h4><p>选择方向，或直接写下想了解的问题</p></div><div class="ai-analysis-time-entry"><span>时间范围</span><div class="ai-reference-analysis-ranges">${rangeButtons}</div><div class="ai-date-range" data-range-scope="analysis"><input type="hidden" name="from" value="${esc(draft.from)}"><input type="hidden" name="to" value="${esc(draft.to)}"><button type="button" class="quiet ai-icon-back ai-time-filter" aria-label="${rangeMode === 'custom' ? '选择自定义日期' : '时间筛选'}" title="时间筛选${draft.from ? `：${esc(draft.from)} 至 ${esc(draft.to)}` : ''}" data-ai-date-range="analysis">${icon('clock')}</button></div></div></div><p class="ai-reference-preset-label">常用分析方向</p><div class="ai-style-pills ai-analysis-presets" role="group" aria-label="分析方向快捷输入">${presetChips(draft.request)}</div><label class="ai-field"><span>分析要求</span><textarea name="request" maxlength="4000" rows="5" placeholder="可留空；例如：总结近期讨论的事项、已达成的约定，以及需要跟进的问题">${esc(draft.request)}</textarea></label><div class="ai-provider-footer"><p class="ai-help">每位联系人独立分析；报告会注明实际读取范围。</p><button type="submit" class="primary" aria-label="开始分析">开始分析 · ${selected.size} 位</button></div></section></form>
