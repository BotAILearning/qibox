...
export function uncertainProfiles(task, state) {
  ...
  for (const item of task.run?.items || []) if (item.status === 'uncertain') {
    const contact = (task.contacts || []).find(c => c.id === item.contact || c.id === item.contactId || c.profileId === item.id || c.profileId === item.profileId);
    const profileId = item.profileId || contact?.profileId || (state.profiles || []).find(p => p.id === item.id || p.contact === item.contact)?.id;
    if (profileId) result.set(profileId, contact?.label || item.label || '联系人');
  }
  for (const c of task.contacts || []) {
    const p = (state.profiles || []).find(p => p.id === c.profileId || p.contact === c.id);
    if (p?.proactiveDelivery?.status === 'uncertain') result.set(p.id, c.label || p.label);
  }
...
export function proactiveTable(state, view = {}) {
  ...
  return `<section class="ap-table-panel"><header class="ap-table-head"><h4>任务列表</h4><span>${all.length} 个任务</span><div class="ap-filters" role="group" aria-label="任务状态">${[['all', '全部'], ...Object.entries(statuses)].map(([key, label]) => `<button type="button" data-proactive-filter="${key}" aria-pressed="${(view.filter || 'all') === key}">${label}</button>`).join('')}</div></header><div class="ap-table-scroll"><table class="ap-task-table"><thead><tr><th>任务名称</th><th>聊天对象</th><th>聊天目标与其他要求</th><th>执行周期</th><th>状态</th><th>最近执行</th><th><span class="sr-only">操作</span></th></tr></thead><tbody>${tasks.map(task => {
    const uncertain = uncertainProfiles(task, state), open = view.menu === task.id;
...
function taskEditor(state, draft) {
  ...
  return `<header class="ap-heading ap-editor-heading"><button type="button" class="secondary" data-proactive-back>← 返回任务列表</button><div><h3>${heading}</h3><p>${readonly ? '已结束或删除的任务仅供查看。' : draft.id ? '调整联系人和执行规则，保存后保持当前任务状态。' : '设置联系人和执行规则，任务建立后会自动执行。'}</p></div></header>${task?.migrationRequired ? `<div class="ap-readiness ap-migration-summary"><b>旧任务已暂停，请核对后再继续。</b><p>${esc(task.migrationSummary || task.legacyScheduleText || task.legacySchedule?.text || '原安排未记录')}</p><p>${task.migrationScheduleMapped ? '已预填可识别的原执行周期，请核对时间和联系人。' : '原周期无法可靠转换，请明确选择下方执行周期；不会自动按一次任务保存。'}</p></div>` : ''}<form id="ai-proactive-form" class="ap-editor"><fieldset ${readonly ? 'disabled' : ''}>
    ...
    <section class="ap-form-card"><label class="ai-field">发送方式<select name="sendMode">${option('segments', '按内容自然分段（1–3 条）', draft.sendMode !== 'single')}${option('single', '只发一条', draft.sendMode === 'single')}</select></label><p>分段使用沟通设置中的消息间隔；中途对方回了话会结合这条回复重新生成剩余内容，你本人接管时则停止剩余段落。</p></section>
    <section class="ap-form-card"><h4>执行安排 <small>北京时间 · UTC+8</small></h4><label class="ai-field">执行周期<select name="cycle" required>${!s.cycle ? option('', '请选择执行周期', true) : ''}${cycles.map(([key, label]) => option(key, label, s.cycle === key)).join('')}</select></label>
