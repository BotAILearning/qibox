...
if (product.buildId !== '0.7.1-debug.001') throw new Error('Unexpected build');
const build = product.buildId, report = path.join(root, 'reports/repair-2026-09-17');
...
for (const file of await files(report)) selection.push('reports/repair-2026-09-17/' + file);
selection.sort();
const manifest = [];
