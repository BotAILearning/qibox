...
if (product.buildId !== '0.8.6-debug.004') throw new Error('Unexpected build: ' + product.buildId);
const build = product.buildId, report = path.join(root, 'reports/development-2026-09-22');
...
await writeFile(path.join(report, 'source-stage-parity-089.json'), JSON.stringify({ build, files: parity }, null, 2));
const selection = [];
...
for (const file of await files(report)) selection.push('reports/development-2026-09-22/' + file);
selection.sort();
const manifest = [];
