...
try {
  ...
  for (const width of [320, 390, 760, 1024, 1440]) {
    ...
    if (width <= 760) {
      ...
      await page.locator('#launch-installed').click(); await page.getByRole('heading', { name: '请用电脑端操作' }).waitFor();
      ...
      await page.getByRole('button', { name: '知道了', exact: true }).click(); page.off('request', listener);
  ...
  await mobilePage.locator('#launch-installed').click(); await mobilePage.getByRole('heading', { name: '请用电脑端操作' }).waitFor();
